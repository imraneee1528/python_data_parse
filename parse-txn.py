...
# Constants for batch size and commit frequency
BATCH_SIZE = 100000
...
def extract_fields_from_log(log_entry):
    try:
        ...
        json_part_match = re.search(r'AnsIptspPstnRequest: (.*)', log_entry)
        ...
        logging.error(f"Error extracting fields from log entry: {e}")
    return None
...
def insert_data(cursor, log_entries):
    try:
        for log_entry in log_entries:
            result = extract_fields_from_log(log_entry)
            if result:
                cursor.execute('''
                INSERT INTO sms_count_txn_202402  (txn_id, msisdn, msisdn_count, message, rn_code, is_unicode, message_count, client_trans_id, msg_len)
        ...
        logging.info(f"Inserted data for {len(log_entries)} log entries")
    except Exception as e:
        logging.error(f"Error inserting data into database: {e}")
        logging.error(f"Failed log entries: {log_entries}")
        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            for line in file:
                ...
                if len(log_entries) >= BATCH_SIZE:
                    insert_data(cursor, log_entries)
                    conn.commit()
                    log_entries = []

            # Insert any remaining log entries
            if log_entries:
                insert_data(cursor, log_entries)
                conn.commit()
    except Exception as e:
        logging.error(f"Error reading log file or connecting to database: {e}")
    finally:
        if conn:
            cursor.close()
            conn.close()
